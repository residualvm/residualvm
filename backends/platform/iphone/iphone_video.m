...
void iPhone_updateOverlayRect(unsigned short* screen, int x1, int y1, int x2, int y2) {
	...
	// TODO: Find out WHY we get rects out-of-bounds, temporary fix for now:
	// Crop the rects back down to within the constraints.
	x2 = MIN(x2, (uint)_overlayWidth);
	y1 = MIN(y1, (uint)_overlayTexHeight);
	x1 = MAX(x1, 0);
	y2 = MAX(y2, 0);
	for (y = y1; y < y2; ++y)
