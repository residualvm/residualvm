...
void OSystem_IPHONE::initSize(uint width, uint height, const Graphics::PixelFormat *format) {
	...
	_offscreen = (byte *)malloc(width * height * sizeof(OverlayColor));
	bzero(_offscreen, width * height * sizeof(OverlayColor));
...
void OSystem_IPHONE::copyRectToScreen(const byte *buf, int pitch, int x, int y, int w, int h) {
	...
	byte *dst = _offscreen + y * _screenWidth * 2 + x * 2;
	if (_screenWidth == pitch && pitch == w)
		memcpy(dst, buf, h * w * 2);
	else {
		do {
			memcpy(dst, buf, w);
			buf += pitch;
...
void OSystem_IPHONE::internUpdateScreen() {
	...
	while (_dirtyRects.size()) {
		...
		if (_overlayVisible){
			if(dirtyRect.top > _overlayHeight){
				//printf("Warning, somehow the rect misses the overlay\n");
				dirtyRect.top = _overlayHeight -1;
			}
			if(dirtyRect.right > _overlayWidth){
				//printf("Warning, somehow the rect misses the overlay\n");
				dirtyRect.top = _overlayWidth -1;
			}
...
void OSystem_IPHONE::drawDirtyRect(const Common::Rect& dirtyRect) {
	...
	uint16 *src = reinterpret_cast<uint16*>(&_offscreen[dirtyRect.top * _screenWidth + dirtyRect.left]);
