...
void Costume::moveHead(bool lookingMode, const Math::Vector3d &lookAt, float rate) {
	if (_joint1Node) {
		...
		y = clampMagnitude(y, _head.maxYaw);
		pt = clampMagnitude(pt, _head.maxPitch);
		r = clampMagnitude(r, _head.maxRoll);
		...
		// hack hack:
		_joint3Node->_animYaw -= _joint3Node->_yaw;
		_joint3Node->_animPitch -= _joint3Node->_pitch;
		_joint3Node->_animRoll -= _joint3Node->_roll;
	}
}
