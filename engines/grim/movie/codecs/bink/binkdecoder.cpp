...
#include "tools/rdft.h"
#include "tools/dct.h"

#include "util.h"

#include "tools/yuv_to_rgb.h"
...
// NB: Might need the MKID-stuff for some platforms later on.
static const uint32 kBIKfID = 'BIKf';
static const uint32 kBIKgID = 'BIKg';
static const uint32 kBIKhID = 'BIKh';
static const uint32 kBIKiID = 'BIKi';
...
BinkDecoder::BinkDecoder(Common::SeekableReadStream *bink) : _bink(bink), _disableAudio(false),
...
// Note: the 25's used here differ from the value of 15 actually found in the BIK-file
// However, 25 fits perfectly with the sound.
bool BinkDecoder::hasTime() const {
	uint32 curTime = g_system->getMillis();
	uint32 frameTime = ((uint64) (_curFrame * 1000 * ((uint64) _fpsDen))) / 25; //fpsnum
...
void BinkDecoder::processData() {
	...
	uint32 frameTime = ((uint64) (_curFrame * 1000 * ((uint64) _fpsDen))) / _fpsNum; //fpsnum
	...
	for (uint32 i = 0; i < _audioTracks.size(); i++) {
		...
		if (frameSize < audioPacketLength)
			error("BINK: Audio packet too big for the frame");
...
void BinkDecoder::audioPacket(AudioTrack &audio) {
	...
	while (!_disableAudio && (audio.bits->pos() < audio.bits->size())) {
		...
		queueSound((byte*)out,audio.blockSize*2);
	...
	for (ctx.blockY = 0; ctx.blockY < blockHeight; ctx.blockY++) {
		...
		for (ctx.blockX = 0; ctx.blockX < blockWidth; ctx.blockX++, ctx.dest += 8, ctx.prev += 8) {
			...
			switch (blockType) {
				...
				default:
					error("BINK: Unknown block type: %d", blockType);
			}
...
void BinkDecoder::load() {
	...
	if (largestFrameSize > fileSize)
		error("BINK: Largest frame size greater than file size");
	...
	if ((_fpsNum == 0) || (_fpsDen == 0))
		error("BINK: Invalid FPS (%d/%d)", _fpsNum, _fpsDen);
...
void BinkDecoder::initAudioTrack(AudioTrack &audio) {
	...
	if (audio.channels > kAudioChannelsMax)
		error("BINK: Too many audio channels: %d", audio.channels);
...
void BinkDecoder::blockScaledRun(DecodeContext &ctx) {
	...
	do {
		...
		i += run;
		if (i > 64)
			error("BINK: Run went out of bounds");
...
void BinkDecoder::blockScaled(DecodeContext &ctx) {
	...
	switch (blockType) {
		...
		default:
			error("BINK: Invalid 16x16 block type: %d\n", blockType);
	}
...
void BinkDecoder::blockMotion(DecodeContext &ctx) {
	...
	if ((prev < ctx.prevStart) || (prev > ctx.prevEnd))
		error("BINK: Copy out of bounds (%d | %d)", ctx.blockX * 8 + xOff, ctx.blockY * 8 + yOff);
...
void BinkDecoder::readRuns(VideoFrame &video, Bundle &bundle) {
	...
	if (decEnd > bundle.dataEnd)
		error("BINK: Run value went out of bounds");
...
void BinkDecoder::readMotionValues(VideoFrame &video, Bundle &bundle) {
	...
	byte *decEnd = bundle.curDec + n;
	if (decEnd > bundle.dataEnd)
		error("BINK: Too many motion values");
...
void BinkDecoder::readBlockTypes(VideoFrame &video, Bundle &bundle) {
	...
	byte *decEnd = bundle.curDec + n;
	if (decEnd > bundle.dataEnd)
		error("BINK: Too many block type values");
...
void BinkDecoder::readPatterns(VideoFrame &video, Bundle &bundle) {
	...
	byte *decEnd = bundle.curDec + n;
	if (decEnd > bundle.dataEnd)
		error("BINK: Too many pattern values");
...
void BinkDecoder::readColors(VideoFrame &video, Bundle &bundle) {
	...
	byte *decEnd = bundle.curDec + n;
	if (decEnd > bundle.dataEnd)
		error("BINK: Too many color values");
		...
		if (bSize) {
			...
			for (uint32 j = 0; j < length2; j++) {
				...
				*dest++ = v;
				...
				if ((v < -32768) || (v > 32767))
					warning("BINK: DC value went out of bounds: %d", v);
			}
