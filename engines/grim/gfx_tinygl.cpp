...
void GfxTinyGL::clearDepthBuffer() {
	memset(_zb->getZBuffer(), 0, _gameWidth * _gameHeight * sizeof(uint32));
}
...
void GfxTinyGL::drawBitmap(const Bitmap *bitmap, int x, int y, uint32 layer) {
	...
	if (bitmap->getFormat() == 1)
		...
		blit(bitmap->getPixelFormat(num), nullptr, (byte *)_zb->getZBuffer(), (byte *)bitmap->getData(num).getRawBuffer(),
